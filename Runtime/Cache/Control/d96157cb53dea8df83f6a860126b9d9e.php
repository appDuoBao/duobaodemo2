	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            ...
            <div class="sleft">
                ...
                <input type="text" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入广告位名称">

                <a class="sch-btn" href="javascript:;" id="search" url="/duobao/app/duobaodemo2/control.php?s=/Adposition/index.html"><i class="btn-search"></i></a>
	...
	<div class="data-table table-striped">
		 ...
		 <table>
			...
			<tbody>
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$adp): $mod = ($i % 2 );++$i;?><tr>

					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($adp["id"]); ?>"></td>

						<td><?php echo ($adp["id"]); ?></td>

						<td><a title="编辑" href="<?php echo U('edit?id='.$adp['id']);?>"><?php echo ($adp["name"]); ?></a></td>

						<td><?php echo ($adp["note"]); ?></td>

                        <td><?php echo (date('Y-m-d H:i:s',$adp["create_time"])); ?></td>

						<td>
							<a title="广告管理" href="<?php echo U('/Control/Ad/index?adp='.$adp['id']);?>">广告管理</a>
                            <a title="编辑" href="<?php echo U('edit?id='.$adp['id']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$adp['id']);?>">删除</a>
                        </td>
