...
<!--未兑奖记录-->
<?php if(count($noExchangeList) > 0): if(is_array($noExchangeList)): $i = 0; $__LIST__ = $noExchangeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="purchase_content nolist">
        ...
        <div class="purchase_right">
            ...
            <div class="purchase_right_choose">参与时间：<?php echo (date('Y/m/d H:i:s', $vo["buy_time"])); ?></div>
...
<?php else: ?>
    <div class="nomore nolist">
        <p>您还没有中奖记录哦</p>
        <p>快<a href="<?php echo U(Index/index);?>">点击这里</a>去夺宝吧</p>
    </div><?php endif; ?>
<!--已兑奖记录-->
<?php if(count($yesExchangeList) > 1): if(is_array($yesExchangeList)): $i = 0; $__LIST__ = $yesExchangeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="purchase_content yeslist">
        <div class="purchase_left">
            <img src="<?php echo (get_cover($vo["order"]["goods_detail"]["cover_id"],'path')); ?>" alt="">
